...
if systemctl is-active --quiet hemi2-1.service; then
    show "hemi2-1.service is currently running. Stopping and disabling it..."
    sudo systemctl stop hemi2-1.service
    sudo systemctl disable hemi2-1.service
else
    show "hemi2-1.service is not running."
fi

cat << EOF | sudo tee /etc/systemd/system/hemi2-1.service > /dev/null
[Unit]
...
sudo systemctl enable hemi2-1.service
sudo systemctl start hemi2-1.service
echo
